...
function fetchIRDBData(zone,cmdt)
{
	var myurl="/RailSAHAY/AcqnPlanMIS";
        ...
        var htmlyrlyldng='<table class="table table-striped table-mis1"><thead><tr><th rowspan="2">Zone</th><th colspan="3">Revenue (In Cr)</th><th colspan="3">Loading (In MT)</th></tr><tr><th>Last Year</th><th>Current Year Projection</th><th>Variation %</th><th>Last Year</th><th>Current Year Projection</th><th>Variation %</th></tr></thead>';
        ...
        var htmlrailcoef='<table class="table table-striped table-mis"><thead><tr><th>Zone</th><th>Division</th><th>Commodity</th><th>Stock</th><th>Total Freight Generation (MT)</th><th>Rail Share (%)</th></tr></thead>';
	$.ajax({
		url : myurl,
		type : "post",
                    ...
                    for(var i=0;i<obj.ZonlPerf.length;i++)
                    {
                        if(obj.ZonlPerf[i].Val2=="")
                        {
                            ...
                            var prevldng=obj.ZonlPerf[i].Val6;
                                ...
                                if(crntrevn=="")
                                {
                                        htmlyrlyldng+='<tr><td>'+obj.ZonlPerf[i].Val1+'</td><td>'+fixDecimal(obj.ZonlPerf[i].Val5)+'</td><td>'+fixDecimal(obj.ZonlPerf[i].Val3)+'</td><td>&nbsp;</td><td>'+fixDecimal(obj.ZonlPerf[i].Val6)+'</td><td>'+fixDecimal(obj.ZonlPerf[i].Val4)+'</td><td>&nbsp;</td></tr>';
                                }
                                else
                                {
                                         ...
                                         for(var j=0;j<obj.ZonlPerf.length;j++)
                                            {
                                                ...
                                                {                                           
                                                    ...
                                                    if(crntrevn=="")
                                                    {
                                                            htmlyrlyldng+='<tr><td>'+obj.ZonlPerf[j].Val2+'</td><td>'+fixDecimal(obj.ZonlPerf[j].Val5)+'</td><td>'+fixDecimal(obj.ZonlPerf[j].Val3)+'</td><td>&nbsp;</td><td>'+fixDecimal(obj.ZonlPerf[j].Val6)+'</td><td>'+fixDecimal(obj.ZonlPerf[j].Val3)+'</td><td>&nbsp;</td></tr>';
                                                    }
                                                    else
                                                    {
                                                            var revn=(((Number(crntrevn)-Number(prevrevn))*100)/Number(prevrevn)).toFixed(2);
                                                            ...
                                                            htmlyrlyldng+='<tr><td>'+obj.ZonlPerf[j].Val2+'</td><td>'+fixDecimal(obj.ZonlPerf[j].Val5)+'</td><td>'+fixDecimal(obj.ZonlPerf[j].Val3)+'</td><td>'+revn1+'</td><td>'+fixDecimal(obj.ZonlPerf[j].Val6)+'</td><td>'+fixDecimal(obj.ZonlPerf[j].Val4)+'</td><td>'+ldng1+'</td></tr>';                                       
                                                    }  
                                                }
                                            }
