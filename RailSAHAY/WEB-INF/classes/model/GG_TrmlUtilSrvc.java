...
public class GG_TrmlUtilSrvc
{
	...
	public String[][] getTrmlList(String si_Sttn) 
	{
		...
		for(int i=0; i<=start1-1; i++)
		{
			try
			{
				...
				strData[i][10]		= RQSAHAYPRTL.getStringItemDef("F_RPWBNUMB",i,"0").trim();
	...
	public String[][] getTrmlProf(String si_Sttn)
	{
		...
		for(int i=0; i<=start1-1; i++)
		{
			try
			{
				...
				strData[i][21]		= RQSAHAYPRTL.getStringItemDef("F_CLBLFRWH",i,"0").trim();
				strData[i][22]		= RQSAHAYPRTL.getStringItemDef("F_TRANID",i,"0").trim();
				strData[i][23]		= RQSAHAYPRTL.getStringItemDef("F_RAKECMDT",i,"0").trim();
				strData[i][24]		= RQSAHAYPRTL.getStringItemDef("F_FRWH",i,"0").trim();
	...
	public String[][] getSMARTFcty(String si_Sttn)
	{
		...
		strData = new String[start1+1][21];
		...
		for(int i=0; i<=start1-1; i++)
		{
			try
			{
				...
				strData[i+1][18]		= RQSAHAYPRTL.getStringItemDef("F_RAKEID",i,"0").trim();
				strData[i+1][19]		= RQSAHAYPRTL.getStringItemDef("F_CLBLUNTS",i,"0").trim();
				strData[i+1][20]		= RQSAHAYPRTL.getStringItemDef("F_DSPTUNTS",i,"0").trim();
            }
            catch(Exception d)
            {
                logger.fatal("Problem in Calling Service RQSAHAYPRTL and filling data into array" + d.toString());
    ...
    public String[][] trackConcern(String si_ConcernId)
    {
            ...
            for(int i=0; i<=start1-1; i++)
            {
                    try
                    {
                            ...
                            strData[i][16]           = RQSAHAYPRTL.getStringItemDef("F_RPWBNUMB2",i,"0").trim();
                            strData[i][17]           = RQSAHAYPRTL.getStringItemDef("F_RPWBNUMB3",i,"0").trim();
                            strData[i][18]           = RQSAHAYPRTL.getStringItemDef("F_RPTGSTTN",i,"0").trim();
                            strData[i][19]           = RQSAHAYPRTL.getStringItemDef("F_RPTGLOCN",i,"0").trim();
    ...
    public String[][] getNodlOfcrDstt(String si_Dstt) 
    {
            String strData[][] = null;   // Variable to be returned as output of function

            logger.info("Entering getNodlOfcrDstt....");
            logger.info("Function called with inputs :");
            logger.info("si_UserID  #"+si_UserID+"#");
            logger.info("si_ClntID  #"+si_ClntID+"#");
            ...
            try
            {
                    ...
                    RQSAHAYPRTL.setString("F_USERID",               0,      si_UserID);
                    RQSAHAYPRTL.setString("F_CLNTID",               0,      si_ClntID);
                    RQSAHAYPRTL.setString("F_FLAG",                 0,      "NODL_OFCR_DSTT");
                    RQSAHAYPRTL.setString("F_HLDGZONE",             0,      si_Dstt);
            }
            catch(Exception e)
            {
                    logger.fatal("Unable to write to buffer : " + e.toString());
	...
	public String[][] getRASSttn(String si_Dvsn)
	{
		String strSttn[][]=null;
		logger.info("getAllZone");	 
		...
		try
		{
			...
			RQFOISPRTLRAS.setString("F_USERID", 0, "SAHAY");
			RQFOISPRTLRAS.setString("F_CLNTID", 0,  "1.1.1.1");
			RQFOISPRTLRAS.setString("F_FLAG", 0,  "RAS_STTN");	
			RQFOISPRTLRAS.setString("F_HLDGZONE", 0,  (si_Dvsn.toUpperCase()).trim());
			...
			if(erorCode1 != null && (!erorCode1.equals("")))
			{
				logger.info("Error from Service:"+erorCode1);
				return strSttn;
			}
			...
			int intRowCount=Integer.parseInt(strData);	
				...
				strSttn = new String[intRowCount][2];

			for (int i=0;i<intRowCount;i++)
			{
				strSttn[i][0]	= RQFOISPRTLRAS.getStringItemDef("F_ORDRBY1",i, "0");
				strSttn[i][1]	= RQFOISPRTLRAS.getStringItemDef("F_ORDRBY2",i, "0");
			}
		
		
			RQFOISPRTLRAS.endSession();
			logger.info("End RQFOISPRTLQRY.");
		}
		catch (Exception e)
		{
		...
		finally
		{
			...
			{
				logger.info("Exception in Finally");
				return strSttn;
			}
        }     
    ...
    public String[][] getRASClstSttn(String si_Clst)
    {
            String strSttn[][]=null;
            logger.info("getAllClstSttn");       
            FOISTuxedo RQFOISPRTLRAS = null;
            ...
            try
            {
                    ...
                    RQFOISPRTLRAS.setString("F_USERID", 0, "SAHAY");
                    RQFOISPRTLRAS.setString("F_CLNTID", 0,  "1.1.1.1");
                    RQFOISPRTLRAS.setString("F_FLAG", 0,  "RAS_CLSTSTTN");      
                    RQFOISPRTLRAS.setString("F_HLDGZONE", 0,  (si_Clst.toUpperCase()).trim());
                    RQFOISPRTLRAS.call("N");
                    logger.info("Called Tux_Call.");
    ...
    public String[][] getRASDvsnClst(String si_Dvsn)
    {
            String strSttn[][]=null;
            logger.info("getAllDvsnClst");       
            FOISTuxedo RQFOISPRTLRAS = null;
            logger.info("si_Dvsn"+si_Dvsn);
            try
            {
            ...
            try
            {
                    ...
                    RQFOISPRTLRAS.setString("F_USERID", 0, "SAHAY");
                    RQFOISPRTLRAS.setString("F_CLNTID", 0,  "1.1.1.1");
                    RQFOISPRTLRAS.setString("F_FLAG", 0,  "RAS_CLST");      
                    RQFOISPRTLRAS.setString("F_HLDGZONE", 0,  (si_Dvsn.toUpperCase()).trim());
                    RQFOISPRTLRAS.call("N");
                    logger.info("Called Tux_Call.");
