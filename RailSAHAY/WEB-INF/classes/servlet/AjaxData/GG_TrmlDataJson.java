...
import model.FeedBackRatings;
import model.GG_TrmlUtilSrvc;

import org.apache.log4j.Logger;

import util.GG_ProjUtil;
...
{	
	...
	public void doPost(HttpServletRequest req, HttpServletResponse	res) throws IOException
	{
	    ...
	    if(si_Optn.equals("RAS_DVSNCLST"))
	    {
	            logger.info("Case: RAS CLUSTERS For DIVISIONS");                      
	            ...
	            try
	            {
	                    String strSrvcData[][];
	                    String strData="";
	                    strSrvcData=obj.getRASDvsnClst(si_Dvsn);
	                    String strCallStts=obj.getCallStts();
	                    String strCallMesg=obj.getCallMesg();
	                    
	                    strData="{\"Stts\":\""+strCallStts+"\", \"ErorMesg\":\""+strCallMesg+"\",\"RASDvsnClst\":[";
	                    int intDataLen=strSrvcData.length;
	                    logger.info("RAS Dvsn Clst Stations Length:"+intDataLen);
	                    
	                    for(int i=0;i<intDataLen;i++)
	                    {       
	                            if(i==(intDataLen-1))
	                                    strData+="{\"Sttn\":\""+strSrvcData[i][0]+"\",\"Name\":\""+strSrvcData[i][1]+"\"}";
	                            else
	                                    strData+="{\"Sttn\":\""+strSrvcData[i][0]+"\",\"Name\":\""+strSrvcData[i][1]+"\"},";
	                    }
	                    strData+="]}";
	    ...
	    if(si_Optn.equals("RAS_CLSTSTTN"))
	    {
	            logger.info("Case: RAS Stations For Clusters");                      
	            String si_Clstr          = req.getParameter("Clst").toUpperCase().trim();       
	            ...
	            try
	            {
	                    String strSrvcData[][];
	                    String strData="";
	                    strSrvcData=obj.getRASClstSttn(si_Clstr);
	                    String strCallStts=obj.getCallStts();
	                    String strCallMesg=obj.getCallMesg();
	                    
	                    strData="{\"Stts\":\""+strCallStts+"\", \"ErorMesg\":\""+strCallMesg+"\",\"RASClstSttn\":[";
	                    int intDataLen=strSrvcData.length;
	                    logger.info("RAS Clst Stations Length:"+intDataLen);
                ...
                if(si_Optn.equals("SAHAY_STATS"))
                {
                        logger.info("Case: SAHAY Statistics");          
                ...
                if(si_Optn.equals("RAS_STTN"))
                {
                        logger.info("Case: RAS Stations");   			
			String si_Dvsn		= req.getParameter("Dvsn").toUpperCase().trim();       
                        ...
                        try
                        {
                                String strSrvcData[][];
                                String strData="";
                                strSrvcData=obj.getRASSttn(si_Dvsn);
                                String strCallStts=obj.getCallStts();
                                String strCallMesg=obj.getCallMesg();
                                
                                strData="{\"Stts\":\""+strCallStts+"\", \"ErorMesg\":\""+strCallMesg+"\",\"RASSttn\":[";
                                int intDataLen=strSrvcData.length;
                                logger.info("RAS Stations Length:"+intDataLen);
                ...
                if(si_Optn.equals("PVT_WGON_STATS"))
                {
                        logger.info("Case: Private Wagon Statistics");  
                        ...
                        try
                        {
                                String strSrvcData[][];
                                String strData="";
                                strSrvcData=obj.getPvtWgonStats();
                                String strCallStts=obj.getCallStts();
                                String strCallMesg=obj.getCallMesg();
                                
                                strData="{\"Stts\":\""+strCallStts+"\", \"ErorMesg\":\""+strCallMesg+"\",\"Stats\":[";
                                int intDataLen=strSrvcData.length;
                                logger.info("Pvt Wagon Stats Length:"+intDataLen);
                                
                                for(int i=0;i<intDataLen;i++)
                                {       
                                        if(i==(intDataLen-1))
                                                strData+="{\"Schm\":\""+strSrvcData[i][0]+"\",\"WgonCont\":\""+strSrvcData[i][1]+"\"}";
                                        else
                                                strData+="{\"Schm\":\""+strSrvcData[i][0]+"\",\"WgonCont\":\""+strSrvcData[i][1]+"\"},";
                                }
                                strData+="]}";
		...
		if(si_Optn.equals("STTN_LIST"))
		{
		    ...
		    strInpt="STATION="+si_Sttn;
		    strInpt = strInpt+"#";
		    
		    
		    strInpt = strInpt.trim().toUpperCase();
		    logger.info("strInpt"+ strInpt);
		     GG_ProjUtil objlog = new GG_ProjUtil();
		     objlog.setSahayInpt(req, "FIND A STATION","/RailSAHAY/pages/SttnList.jsp", strInpt);
			...
			try
			{
				...
				{							
					...
					strLat=strSrvcData[i][5];
					strLng=strSrvcData[i][6];
					strLSPName=strSrvcData[i][7];
					strContPers=strSrvcData[i][8];
					strMobl=strSrvcData[i][9];
					strEmail=strSrvcData[i][10];

					if(i==(intDataLen-1))
						strData+="{\"Sttn\":\""+strSttn+"\",\"Dvsn\":\""+strDvsn+"\",\"Zone\":\""+strZone+"\",\"Stat\":\""+strStat+"\",\"Fcty\":\""+strFcty+"\",\"Lat\":\""+strLat+"\",\"Lan\":\""+strLng+"\",\"LSPName\":\""+strLSPName+"\",\"ContPers\":\""+strContPers+"\",\"Mobl\":\""+strMobl+"\",\"Email\":\""+strEmail+"\"}";
					else
						strData+="{\"Sttn\":\""+strSttn+"\",\"Dvsn\":\""+strDvsn+"\",\"Zone\":\""+strZone+"\",\"Stat\":\""+strStat+"\",\"Fcty\":\""+strFcty+"\",\"Lat\":\""+strLat+"\",\"Lan\":\""+strLng+"\",\"LSPName\":\""+strLSPName+"\",\"ContPers\":\""+strContPers+"\",\"Mobl\":\""+strMobl+"\",\"Email\":\""+strEmail+"\"},";				
				}
				strData+="]}";
		...
		if(si_Optn.equals("SMART_FCTY"))
		{
		    ...
		    logger.info("strInpt"+ strInpt);
		     GG_ProjUtil objlog = new GG_ProjUtil();
		     objlog.setSahayInpt(req, "SMART","/RailSAHAY/pages/SMART.jsp", strInpt);
			...
			try
			{
				...
				{							
					if(i==(intDataLen-1))
                                            strData+="{\"LSPName\":\""+strSrvcData[i][0]+"\",\"GSTN\":\""+strSrvcData[i][1]+"\",\"Addr\":\""+strSrvcData[i][2]+"\",\"City\":\""+strSrvcData[i][3]+"\",\"Stat\":\""+strSrvcData[i][4]+"\",\"Pin\":\""+strSrvcData[i][5]+"\",\"ContPers\":\""+strSrvcData[i][6]+"\",\"Mob\":\""+strSrvcData[i][7]+"\",\"Email\":\""+strSrvcData[i][8]+"\",\"FacType\":\""+strSrvcData[i][9]+"\",\"FacDesc\":\""+strSrvcData[i][10]+"\",\"AddlInfo\":\""+strSrvcData[i][11]+"\",\"FctyContPers\":\""+strSrvcData[i][12]+"\",\"FctyMob\":\""+strSrvcData[i][13]+"\",\"FctyEmail\":\""+strSrvcData[i][14]+"\",\"SrvgCont\":\""+strSrvcData[i][15]+"\",\"LSPType\":\""+strSrvcData[i][16]+"\",\"URL\":\""+strSrvcData[i][17]+"\",\"TotlCpty\":\""+strSrvcData[i][18]+"\",\"AvblCpty\":\""+strSrvcData[i][19]+"\",\"AsOn\":\""+strSrvcData[i][20]+"\"}";
					else
                                            strData+="{\"LSPName\":\""+strSrvcData[i][0]+"\",\"GSTN\":\""+strSrvcData[i][1]+"\",\"Addr\":\""+strSrvcData[i][2]+"\",\"City\":\""+strSrvcData[i][3]+"\",\"Stat\":\""+strSrvcData[i][4]+"\",\"Pin\":\""+strSrvcData[i][5]+"\",\"ContPers\":\""+strSrvcData[i][6]+"\",\"Mob\":\""+strSrvcData[i][7]+"\",\"Email\":\""+strSrvcData[i][8]+"\",\"FacType\":\""+strSrvcData[i][9]+"\",\"FacDesc\":\""+strSrvcData[i][10]+"\",\"AddlInfo\":\""+strSrvcData[i][11]+"\",\"FctyContPers\":\""+strSrvcData[i][12]+"\",\"FctyMob\":\""+strSrvcData[i][13]+"\",\"FctyEmail\":\""+strSrvcData[i][14]+"\",\"SrvgCont\":\""+strSrvcData[i][15]+"\",\"LSPType\":\""+strSrvcData[i][16]+"\",\"URL\":\""+strSrvcData[i][17]+"\",\"TotlCpty\":\""+strSrvcData[i][18]+"\",\"AvblCpty\":\""+strSrvcData[i][19]+"\",\"AsOn\":\""+strSrvcData[i][20]+"\"},";
				}
		...
		if(si_Optn.equals("CRT"))
		{
			...
			GG_TrmlUtilSrvc obj=new GG_TrmlUtilSrvc(si_UserID,si_ClntID);
                        ...
                        strInpt="LOCN FLAG="+si_LocnFlag;
		        strInpt = strInpt+"#";
                        strInpt=strInpt+"LOCN="+si_Locn;
                        strInpt = strInpt+"#";
                        strInpt=strInpt+"COMMODITY="+si_Cmdt;
                        strInpt = strInpt+"#";
                        strInpt=strInpt+"LAT="+si_Lat;
                        strInpt = strInpt+"#";
                        strInpt=strInpt+"LONG="+si_Lng;
                        strInpt = strInpt+"#";
                        
                        strInpt = strInpt.trim().toUpperCase();
                        logger.info("strInpt"+ strInpt);
		         GG_ProjUtil objlog = new GG_ProjUtil();
                         objlog.setSahayInpt(req, "CRT LOCATIONS","/RailSAHAY/pages/CRTLocn.jsp", strInpt);
			...
			try
			{
				...
				{							
					    if(i==(intDataLen-1))
					        strData+="{\"Zone\":\""+strSrvcData[i][0]+"\",\"ZoneName\":\""+strSrvcData[i][1]+"\",\"Dvsn\":\""+strSrvcData[i][2]+"\",\"DvsnName\":\""+strSrvcData[i][3]+"\",\"Sttn\":\""+strSrvcData[i][4]+"\",\"SttnName\":\""+strSrvcData[i][5]+"\",\"Dstt\":\""+strSrvcData[i][6]+"\",\"Stat\":\""+strSrvcData[i][7]+"\",\"AgCont\":\""+strSrvcData[i][8]+"\",\"TrckCont\":\""+strSrvcData[i][9]+"\",\"WHCont\":\""+strSrvcData[i][10]+"\",\"LabCont\":\""+strSrvcData[i][11]+"\",\"Lat\":\""+strSrvcData[i][12]+"\",\"Lan\":\""+strSrvcData[i][13]+"\"}";
					    else
					    strData+="{\"Zone\":\""+strSrvcData[i][0]+"\",\"ZoneName\":\""+strSrvcData[i][1]+"\",\"Dvsn\":\""+strSrvcData[i][2]+"\",\"DvsnName\":\""+strSrvcData[i][3]+"\",\"Sttn\":\""+strSrvcData[i][4]+"\",\"SttnName\":\""+strSrvcData[i][5]+"\",\"Dstt\":\""+strSrvcData[i][6]+"\",\"Stat\":\""+strSrvcData[i][7]+"\",\"AgCont\":\""+strSrvcData[i][8]+"\",\"TrckCont\":\""+strSrvcData[i][9]+"\",\"WHCont\":\""+strSrvcData[i][10]+"\",\"LabCont\":\""+strSrvcData[i][11]+"\",\"Lat\":\""+strSrvcData[i][12]+"\",\"Lan\":\""+strSrvcData[i][13]+"\"},";
                                }
				strData+="]}";
		...
		if(si_Optn.equals("PFT"))
		{
		    ...
		    logger.info("strInpt"+ strInpt);
		     GG_ProjUtil objlog = new GG_ProjUtil();
		     objlog.setSahayInpt(req, "PFT LOCATIONS","/RailSAHAY/pages/PFTLocn.jsp", strInpt);
		...
		if(si_Optn.equals("LDNG"))
		{
		    ...
		    logger.info("strInpt"+ strInpt);
		     GG_ProjUtil objlog = new GG_ProjUtil();
		     objlog.setSahayInpt(req, "LOADING LOCATIONS","/RailSAHAY/pages/LdngLocn.jsp", strInpt);
		...
		if(si_Optn.equals("ULDG"))
		{
		    ...
		    logger.info("strInpt"+ strInpt);
		     GG_ProjUtil objlog = new GG_ProjUtil();
		     objlog.setSahayInpt(req, "UNLOADING LOCATIONS","/RailSAHAY/pages/UldgLocn.jsp", strInpt);
		...
		if(si_Optn.equals("TTROUT"))
		{
		    ...
		    logger.info("strInpt"+ strInpt);
		     GG_ProjUtil objlog = new GG_ProjUtil();
		     objlog.setSahayInpt(req, "TIME TABLED ROUTES","/RailSAHAY/pages/TimeTblTran.jsp", strInpt);
	    ...
	    if(si_Optn.equals("TRACK_CONCERN"))
	    {
	            ...
	            try
	            {
	                    ...
	                    {                                                       
	                            if(i==(intDataLen-1))
                                    strData+="{\"Type\":\""+strSrvcData[i][2]+"\",\"Ctgr\":\""+strSrvcData[i][3]+"\",\"SubCtgr\":\""+strSrvcData[i][4]+"\",\"FName\":\""+strSrvcData[i][5]+"\",\"LName\":\""+strSrvcData[i][6]+"\",\"Phone\":\""+strSrvcData[i][7]+"\",\"Email\":\""+strSrvcData[i][8]+"\",\"Zone\":\""+strSrvcData[i][9]+"\",\"Dvsn\":\""+strSrvcData[i][10]+"\",\"Sttn\":\""+strSrvcData[i][11]+"\",\"Rmrk\":\""+strSrvcData[i][12]+"\",\"UpdtTime\":\""+strSrvcData[i][13]+"\",\"CncrStts\":\""+strSrvcData[i][14]+"\",\"SttsRmrk\":\""+strSrvcData[i][15]+"\",\"FileName\":\""+strSrvcData[i][16]+"\",\"RespTime\":\""+strSrvcData[i][17]+"\",\"RespFile\":\""+strSrvcData[i][18]+"\",\"RespText\":\""+strSrvcData[i][19]+"\"}";
	                            else
                                    strData+="{\"Type\":\""+strSrvcData[i][2]+"\",\"Ctgr\":\""+strSrvcData[i][3]+"\",\"SubCtgr\":\""+strSrvcData[i][4]+"\",\"FName\":\""+strSrvcData[i][5]+"\",\"LName\":\""+strSrvcData[i][6]+"\",\"Phone\":\""+strSrvcData[i][7]+"\",\"Email\":\""+strSrvcData[i][8]+"\",\"Zone\":\""+strSrvcData[i][9]+"\",\"Dvsn\":\""+strSrvcData[i][10]+"\",\"Sttn\":\""+strSrvcData[i][11]+"\",\"Rmrk\":\""+strSrvcData[i][12]+"\",\"UpdtTime\":\""+strSrvcData[i][13]+"\",\"CncrStts\":\""+strSrvcData[i][14]+"\",\"SttsRmrk\":\""+strSrvcData[i][15]+"\",\"FileName\":\""+strSrvcData[i][16]+"\",\"RespTime\":\""+strSrvcData[i][17]+"\",\"RespFile\":\""+strSrvcData[i][18]+"\",\"RespText\":\""+strSrvcData[i][19]+"\"},";
	                    }
	                    strData+="]}";
	    ...
	    if(si_Optn.equals("NODL_OFCR_DSTT"))
	    {
	            logger.info("Case: NODL_OFCR_DSTT");                    
	            String si_Dstt     = req.getParameter("Dstt").toUpperCase().trim();
	            
	            logger.info("si_Dstt:"+si_Dstt);
	            ...
	            try
	            {
	                    String strSrvcData[][];
	                    String strData="";
	                    strSrvcData=obj.getNodlOfcrDstt(si_Dstt);
	                    ...
	                    {                                                       
	                            if(i==(intDataLen-1))
	                            strData+="{\"Dvsn\":\""+strSrvcData[i][0]+"\",\"Ofcr\":\""+strSrvcData[i][1]+"\",\"Mob\":\""+strSrvcData[i][2]+"\",\"LL\":\""+strSrvcData[i][3]+"\"}";
	                            else
	                            strData+="{\"Dvsn\":\""+strSrvcData[i][0]+"\",\"Ofcr\":\""+strSrvcData[i][1]+"\",\"Mob\":\""+strSrvcData[i][2]+"\",\"LL\":\""+strSrvcData[i][3]+"\"},";
	                    }
	                    strData+="]}";
	    ...
	    if(si_Optn.equals("SAHAY_RTNG"))
	    {
	            logger.info("Case: SAHAY Ratings");          
	        String si_Rtng     = req.getParameter("Rating");
	            FeedBackRatings obj=new FeedBackRatings();
	            ...
	            try
	            {
	                    ...
	                    strSrvcData=obj.getRatingComments(si_UserID,si_ClntID,si_Rtng);
	                    String strCallStts=obj.getStrCallStts();
	                    String strCallMesg=obj.getStrCallMesg();
	                  
	                strData="{\"Stts\":\""+strCallStts+"\", \"ErorMesg\":\""+strCallMesg+"\",\"Review\":[";
	                int intDataLen=strSrvcData.length;
	                logger.info("Reviews List:"+intDataLen);
                        
	                for(int i=0;i<intDataLen;i++)
	                {                                                       
	                        if(i==(intDataLen-1))
	                        strData+="{\"Name\":\""+strSrvcData[i][0]+"\",\"Rating\":\""+strSrvcData[i][1]+"\",\"Text\":\""+strSrvcData[i][2]+"\"}";
	                        else
	                        strData+="{\"Name\":\""+strSrvcData[i][0]+"\",\"Rating\":\""+strSrvcData[i][1]+"\",\"Text\":\""+strSrvcData[i][2]+"\"},";
	                } 
	                strData+="]}";
