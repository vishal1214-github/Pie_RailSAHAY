		...
		if(stts=="S")
                {
                    ...
                    if(measure=="AP"){
                        ...
                        printTable("example1","MEASURES FOR PROGRESS: ACTION PLAN","ZONE: "+zone+", COMMODITY: "+cmdt);
                    }
                    if(measure=="IN"){
                        $("#divMeasure").html(htmlin);
                        printTable("example2","MEASURES FOR PROGRESS: INITIATIVES TAKEN","ZONE: "+zone+", COMMODITY: "+cmdt);
                    }
                    if(measure=="IC"){
                        $("#divMeasure").html(htmlic);
                        printTable("example4","MEASURES FOR PROGRESS: RAIL COEFFICIENT","ZONE: "+zone+", COMMODITY: "+cmdt);
                    }
                    if(measure=="IO"){
                        $("#divMeasure").html(htmlio);
                        printTable("example3","MEASURES FOR PROGRESS: INDUSTRY OUTREACH","ZONE: "+zone+", COMMODITY: "+cmdt);
                    }
                    if(measure=="FC"){
                        $("#divMeasure").html(htmlfc);
                        printTable("example5","MEASURES FOR PROGRESS: FREIGHT FORWARDERS (CURRENT)","ZONE: "+zone+", COMMODITY: "+cmdt);
                    }
                    if(measure=="FP"){
                        $("#divMeasure").html(htmlfp);
                        printTable("example6","MEASURES FOR PROGRESS: FREIGHT FORWARDERS (PROPOSED)","ZONE: "+zone+", COMMODITY: "+cmdt);
                    }
