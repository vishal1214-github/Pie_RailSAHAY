...
function showBrkup(brktype,trmltype)
{
	...
	switch(trmltype)
	{
		case 'PFT':
			trmltypeindx=2;
			trmltitl="PFT (FOR "+GG_RakeCmdtName+")";
			break;
		case 'CRT':
			trmltypeindx=3;
			trmltitl="CRT (FOR "+GG_RakeCmdtName+")";
			break;
		case 'LDNG':
			trmltypeindx=4;
...
function fetchTrmlSrchRakeCmdt(numcmdt)
{
	...
	$.ajax({
		...
		success : function(data)
		{
			...
			if(DataFlag == "N")
			{
				cuteToast({
				  type: "success",
				  message: "There's some problem, please try again later",
				  timer: 5000
				})
				return false;
			}
    ...
    $(document).ready(function(){
     $("#selTrtyType").change(function(){
      var locntype=$("#selTrtyType").val();
