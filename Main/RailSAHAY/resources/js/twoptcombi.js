...
var sttnstr='';
   var cmdtstr='';
   for (var i=0; i < aGrupRakeCmdt.length;++i){
        cmdtstr+= '<option value="'+aGrupRakeCmdt[i]+'" />'; // Helplist for station
   }
...
function fetchCombi()
{
	...
	else
	{
		...
		{
		cmdt=cmdtdesc.substring(cmdtdesc.indexOf("-")+1);
		}
		else
		{
	...
	fetchData(locntype,locn,cmdt,raketype,owiwflag);
...
function fetchData(locntype,locn,cmdt,raketype,owiwflag)
{
	...
	$.ajax({
		...
		{
			custPrompt("No Two-Point/Multi-Point Combination Exists for the Input Criteria");			
		}
