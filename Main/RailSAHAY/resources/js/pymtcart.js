	...
	$.ajax({
		...
		success : function(data) {
			...
			for(var i=0;i<obj.Breakup.length;i++)
			{
				...
				if(chrgname=='WRF GST Adjustment Amount')
				{
					if(Number(amnt)>0)
					{
						G_WRFTaxAjstAmnt=Number(amnt);					
						G_Waiver+=Number(amnt);
					}
				}
				if(chrgname=="Total GST")
				...
				if((chrgname=="Total GST") || (chrgname=="Total Amount") || (chrgname=="Net Payable Amount") || (chrgname=="WRF Adjustment Amount")|| (chrgname=="WRF GST Adjustment Amount")|| (chrgname=="Amount"))
					htmchrgbrkup+='<tr><td class="lbl">'+chrgname+'</td><td class="val" style="text-align:right;"><i class="fas fa-rupee-sign"></i>&nbsp;'+amnt+'</td></tr>';
			}
...
function addToCart(chrgid, sttn, sttnname, dvsn, zone, custcode, invcid, fnotid, fnotnumb, fnotdate, custdmrgid, trxnid, chrgtype, chrgtypedesc, iwowflag, invcdate, cnfmdate, acrldate, amnt,waiver, gst,  netamnt, rlygstin, cnsrgstin, cnsggstin,wrfajstflag, wrfajstamnt,wrftaxajstamnt,hndgagnt)
{
		...
		success : function(data) {
			...
			if(stts=="F")
			{
				if(hndgagnt=='')
					...
					$("#cartSecGSTBodyFtr").html('<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert">&times;</button><strong>Cart Update Problem!</strong><br/>'+erormesg+'</div>');
...
{
	addToCart(G_ChrgId, G_Sttn, G_SttnName, G_Dvsn, G_Zone, G_CustCode, G_InvcId, G_FNotId, G_FNotNumb, G_FNotDate,G_CustDmrgId, G_TrxnId, G_ChrgType, G_ChrgTypeDesc, G_IWOWFlag, G_InvcDate, G_CnfmDate, G_AcrlDate, G_Amnt,G_Waiver, G_GstAmnt,G_NetAmnt, G_RlyGstin, G_CnsrGstin, G_CnsgGstin,G_WRFAjstFlag,G_WRFAjstAmnt,G_WRFTaxAjstAmnt,G_HndgAgnt);
}
function showCart()
{
			...
			else
			{
				...
				else 
				{
					for(var i=0;i<obj.Data.length;i++)
					{
						...
						htmlstr+='<td rowspan="2"><span class="amnt float-right"><i class="fas fa-rupee-sign"></i>&nbsp;'+obj.Data[i].NetAmnt+'</span></td>';
						htmlstr+='</tr>';
						htmlstr+='<tr>';
						...
						htmlstr+='</tr></table>';		
						htmlstr+='<button class="btn btn-sm btn-warning del-icon" onclick="showDelConfirmationBox(\''+obj.Data[i].ChrgId+'\');"><i class="fas fa-times"></i></button>';
...
$('body').on('click', 'button.btn-sec-data', function() {

    var chrgid=$("#SecCartChrgId").val();
    var sttn=$("#SecCartSttn").val();
    var custcode=$("#SecCartCustCode").val();
    var hndgagnt=$("#SecCartHndgAgnt").val();
    var rlygstin=$("#SecCartRlyGstIn").val();
    var hndgagntgstin=$("#selSecCartGSTIN").val();
    var sttnname=$("#SecCartSttnName").val();
    var dvsn=$("#SecCartDvsn").val();
    var zone=$("#SecCartZone").val();
    var fnotid=$("#SecCartFnotId").val();
    var fnotnumb=$("#SecCartFnotNumb").val();
    var fnotdate=$("#SecCartFnotDate").val();
    var invcid=$("#SecCartInvcId").val();
    var invcnumb=$("#SecCartInvcNumb").val();
    var invcdate=$("#SecCartInvcDate").val();
    var custdmrgid=$("#SecCartCustDmrgId").val();
    var iwowflag=$("#SecCartIWOWFlag").val();
    var cnfmdate=$("#SecCartCnfmDate").val();
    var acrldate=$("#SecCartAcrlDate").val();
    var amnt=$("#SecCartAmnt").val();
    var chrgtype=$("#SecCartChrgType").val();
    var chrgtypedesc=$("#SecCartChrgTypeDesc").val();
    var chrgtypecode=$("#SecCartChrgTypeCode").val();
    var trxnid=$("#SecCartTrxnId").val();

    if((chrgtype=='D') || (chrgtype=='W'))
    {
	showCartSecChrgDtls(this, chrgid, sttn, sttnname, dvsn, zone, custcode, trxnid, invcid, fnotid, fnotnumb, fnotdate, custdmrgid, trxnid, chrgtype,chrgtypecode, chrgtypedesc, iwowflag, invcdate, invcnumb, cnfmdate, acrldate, amnt, rlygstin, hndgagntgstin, hndgagntgstin,hndgagnt);
    } 
    if(chrgtype=='L')
    {
	fetchCartSecLoclChrgDtls(this, chrgid, sttn, sttnname, dvsn,zone,custcode, invcid, fnotid, fnotnumb, fnotdate, custdmrgid, trxnid, chrgtype, chrgtypecode, chrgtypedesc, iwowflag, invcdate, invcnumb, amnt, cnfmdate, acrldate, rlygstin, hndgagntgstin, hndgagntgstin,hndgagnt)
    }	
    
});
function showCartSecChrgDtls(link, chrgid, sttn, sttnname, dvsn, zone, custcode, trxnid, invcid, fnotid, fnotnumb, fnotdate, custdmrgid, trxnid, chrgtype,chrgtypecode, chrgtypedesc, iwowflag, invcdate,invcnumb, cnfmdate, acrldate, amnt, rlygstin, cnsrgstin, cnsggstin,hndgagnt)
{	
	$("#btnAddSecCart").hide();
	var chrgtypedesc='';

	$("#cartSecGSTBody1").html('<div class="spinner-border text-danger mt-2 ml-2"></div>');
	htmlstr='';
	var htmlstr='<table class="table table-striped table-bordered table-cart" style="margin-bottom:0px;border:0px;">';
   var dmrgwrfgtype='';
   var optn="";
...
function fetchCartSecLoclChrgDtls(link, chrgid, sttn, sttnname, dvsn, zone, custcode, invcid, fnotid, fnotnumb, fnotdate, custdmrgid, trxnid, chrgtype, chrgtypecode, chrgtypedesc, iwowflag, invcdate, invcnumb, amnt, cnfmdate, acrldate, rlygstin, cnsrgstin, cnsggstin,hndgagnt)
{
	var loclchrggstin='';
	var htmchrgbrkup='<table class="table table-striped table-cart">';
	$("#cartSecGSTBody1").html('<div class="spinner-border text-danger mt-2 ml-2"></div>');
	...
	$.ajax({
		...
		success : function(data) {
			...
			htmchrgbrkup+='</table>';			
			$("#cartSecGSTBody1").html(htmchrgbrkup);
			$("#btnAddSecCart").show();			
